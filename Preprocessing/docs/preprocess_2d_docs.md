...
4. **Crop & Pad**: It mathematically crops the very center of the image to your exact `target_size` (e.g., `256x256`) via `norm.center_crop_pad`.

### Step 5: Saving
Finally, it saves the aligned, cropped, `[0,1]` normalized 2D numpy arrays into the output folder as `.npy` files. These files are now 100% ready to be fed directly into a PyTorch or TensorFlow Deep Learning model!
