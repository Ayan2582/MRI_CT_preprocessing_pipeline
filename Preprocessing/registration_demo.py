...
def prepare_candidate(cand):
    ...
    baseline_old_mri_res = img_proc.resample_mri_to_ct_grid(mri_corrected, ct_res, default_pixel_value=0.0)
...
def process_candidate(cand):
    ...
    mi_baseline_old = mattes_mi_score(ct_slice_img, baseline_old_slice_img)
    mi_baseline = mattes_mi_score(ct_slice_img, baseline_mri_slice_img)
    print(f"    MI  baseline(old)={mi_baseline_old:.4f}  baseline(v2)={mi_baseline:.4f}  "
    ...
    fusion_panel(axes[2, 0], ct_res_disp, baseline_old_disp,
                 f"5. Baseline OLD (direction-hack)  MI={mi_baseline_old:.3f}")
    fusion_panel(axes[2, 1], ct_res_disp, baseline_disp,
                 f"6. Baseline v2 (GEOMETRY translation)  MI={mi_baseline:.3f}")
    axes[2, 2].axis("off")
    axes[2, 2].text(
        0.0, 0.5,
        ...
        f"MI  old→v2: {mi_baseline_old:.3f} → {mi_baseline:.3f}",
        fontsize=9, va="center", wrap=True,
    )

    fusion_panel(axes[3, 0], ct_res_disp, baseline_disp, f"7. Baseline v2 fusion  MI={mi_baseline:.3f}")
    fusion_panel(axes[3, 1], ct_res_disp, rigid_disp, f"8. Rigid (best of {N_STARTS})  MI={mi_rigid:.3f}")
    fusion_panel(axes[3, 2], ct_res_disp, affine_disp, f"9. Affine (best of {N_STARTS})  MI={mi_affine:.3f}")
    ...
    return {
        ...
        "orientation": cand["orientation"],
        "slice_index": z,
        "mi_baseline_old": mi_baseline_old,
